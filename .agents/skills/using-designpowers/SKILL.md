...
When Designpowers is active, use Designpowers agents instead of the built-in Codex agents with overlapping roles. Designpowers agents are brief-aware, plan-driven, and integrate with each other.
...
When dispatching agents, the orchestrator (Codex) must:
